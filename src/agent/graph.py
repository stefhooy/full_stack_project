"""The agent graph: a minimal, fully-explicit tool-calling loop with one
tool (run_sql) and a visible self-correction path.

    START -> agent -> [has tool_calls?] -> execute_tools -> agent -> ... -> END
                    \\-> no tool_calls -> END
