...
### The question that reframed the whole approach

Asked directly: "is there a better solution? What would an AI engineer
do?" -- a fair challenge, because the honest answer was that Slice 58's
fix was still the *weaker* of two well-known approaches to this problem
class. It made the request for accuracy more specific (name the exact
field, forbid the exact mechanism observed), but it was still a request
-- a probabilistic ask the model could, and twice did, fail to honor.

The reframe: if a fact already exists in code (`stats_result`, straight
from `run_stats`), never let the model regenerate that fact in free
text at all. Detection-after-the-fact (the original plan discussed
before this reframe: a new graph node, a retry loop, a string-matching
heuristic) is strictly weaker than prevention, for one concrete reason
-- detection has to correctly judge a moving target (arbitrary model
phrasing) and can always miss a paraphrase or false-positive on an
honest hedge. Prevention has no target to miss: if the model never
writes the sentence, there is no sentence to get wrong.

This is "make invalid states unrepresentable," applied to LLM output
instead of a type system. The general pattern -- **render facts
deterministically from structured tool output; let the LLM narrate,
never restate** -- is exactly how a lot of real production
analytics/BI copilots handle this same class of risk, and it's the
answer worth having ready for an interview: prompting reduces
hallucination *rate*; only removing the model's opportunity to generate
the fact removes the *failure mode*.

### Why simply appending a correct block isn't enough

The naive version of "render facts in code" -- print the correct
outliers list, then just tack the model's full original paragraph on
after it -- doesn't actually fix anything: if the model's own paragraph
*still* says "PUBG is also an outlier," the user now sees both the
correct block and the wrong claim sitting side by side. The fix has to
shrink what the model is even asked to produce, not just add a correct
answer next to a possibly-wrong one.

### The actual design

`ANALYSIS_TOOL_GUIDANCE` (`prompts.py`) now tells the model, for
`mode="outliers"` specifically, that its final answer must not name any
row, value, or z-score at all -- only a short (1-2 sentence),
completely name-free interpretive comment. The prompt says outright
that asking it to restate the list accurately has already failed twice
for real, so it's simply not the model's job anymore. This shrinks the
model's writing surface from "the whole factual answer" to "one
sentence of color commentary with nothing left in it to hallucinate."

`_render_outliers_fact_block()` (`graph.py`) is the actual fact: an
f-string over `stats_result`'s real `outliers` list and `z_threshold`,
naming the real outlier(s) with their real z-scores, or honestly
stating nothing clears the threshold. This text is never generated by
an LLM call -- it cannot be wrong in a way a prompt could fail to
prevent, because there is no prompt involved in producing it at all.

`_compose_outliers_answer()` combines the two: fact block first, then
the model's commentary. One defensive backstop, since prompt compliance
still isn't literally 100%: if the model's commentary *still* names an
unauthorized row from a companion `run_sql` query (the exact mechanism
behind the original hallucination -- the model running a "top N" query
to build a display table, then treating every row in it as a flagged
outlier), the commentary is swapped wholesale for a safe, generic,
name-free sentence rather than surgically edited. Wholesale replacement
over clause-level editing was a deliberate simplicity choice: trying to
remove just the offending clause from free text risks leaving broken
grammar behind (a dangling "and," an incomplete sentence), and the fact
block above it is completely unaffected either way, so the backstop
firing costs nothing but a slightly blander sentence, never
correctness.

Wired into `_result_from_state()` -- the one real choke point both
`/ask` and `/ask/stream` already funnel every answer through -- so both
endpoints get the guarantee automatically, no duplicated logic per
endpoint.

### A genuinely nice side effect: zero eval-harness changes needed

`analysis_ccu_outliers`/`analysis_price_outliers`/`analysis_discount_outliers`'s
existing golden-question checks (`contains_text(name)` for a real
outlier, `route_is` alone for the honest "nothing clears the threshold"
case) check the final `answer` string -- which still contains the real
name, now via the fact block instead of the model's own prose. These
checks are, if anything, now *more* reliably satisfied than before,
since the name's presence in the answer no longer depends on the model
writing it correctly at all.

### Verified for real

Added `test_graph_outliers_answer_composition.py`: 7 tests, pure
functions of `stats_result` and plain strings, zero LLM calls needed --
the fact block's exact rendering (one outlier, several, the honest
empty case) and the composition/backstop logic (clean commentary passes
through untouched; an unauthorized name gets swapped for the safe
fallback while the fact block stays intact regardless; a real outlier's
own name mentioned in commentary is never mistakenly flagged; no
companion query means nothing to check against). `ruff check .` and
`uv run mypy src` clean, 209/209 tests pass (202 + 7 new).

### What's actually guaranteed versus what still needs a live check

Worth being precise about the difference: the *naming* failure mode is
now architecturally impossible for the primary path, not just less
likely -- that text is simply not something the model writes anymore,
so there's nothing left to verify live for that part specifically. The
defensive backstop is different: it still depends on the model's
short commentary text and a companion-query heuristic, so *that* path's
real effectiveness is exactly as unconfirmed as any other prompt-facing
logic until a live run exercises it. The next `run_evals.yml` run should
show `analysis_ccu_outliers`/`analysis_discount_outliers` with clean
judge scores now that the model has nothing factual left to get wrong
in its own text -- if the backstop fires at all, that's itself a signal
worth reading closely, not just accepting silently.

### Open questions (new)

- Whether the same "render facts, narrate around them" pattern should
  extend to `compare_two_groups` and `describe` too. Not done here,
  scoped deliberately to `outliers` only (the one mode with an actual
  observed, repeated hallucination) -- `compare_two_groups`'s risk
  shape is different (mislabeling an existing group, already covered by
  Slice 4/30's own check) and `describe`'s only real gap found so far
